---
name: kanban-commit
description: Commit task changes from a worktree onto the base branch (read from KANBAN_BASE_BRANCH env var). Use when finishing a kanban task to stage, commit, and cherry-pick changes back to the base ref.
---

# Kanban Commit

Commit the working changes from a kanban task worktree onto the base branch.

## When to use

This skill runs automatically as part of the kanban orchestrator workflow. You should run it when finishing a kanban task to move completed work from the task worktree back to the project's base branch.
...
5. If P has uncommitted changes, stash them:
   ```
   git -C P stash push -u -m "kanban-pre-cherry-pick"
   ```
...
8. If step 5 created a new stash entry, restore that stash with:
   ```
   git -C P stash pop <stash-ref>
   ```
...
- [kanban-orphan-commit-recovery](../kanban-orphan-commit-recovery/SKILL.md) — recover dangling commits when this skill fails to cherry-pick
- To debug issues, check the orchestrator logs in `/tmp/kanban-*/*.log`
